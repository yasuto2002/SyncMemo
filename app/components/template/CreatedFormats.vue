  ...
  <form class="bg-white p-[5%] w-[47%] rounded-[10px] h-full">
    ...
    <div class="w-[80%] m-auto">
      ...
      <input
        ...
        name=""
        id=""
        class="bg-white outline-none border-[1px] border-[#ACA5A5] border-solid w-full mb-[3vw] rounded-[10px] h-10"
        v-model="name"
      /><br />
      <input
        type="checkbox"
      ...
      />
      <label for="validity" :class="{ 'opacity-60': !loginStatus }"
        >パスワードを有効にする</label
      >
      <p class="text-[#ff0000]" v-if="!loginStatus">
        ログインユーザーのみパスワードを有効にできます
      </p>
      <p
        class="mt-[3vw]"
        v-bind:class="{ 'text-[#F3F5F4]': !checked || !loginStatus }"
      >
        パスワード
      </p>
      ...
      <input
        ...
        name=""
        id=""
        class="bg-white outline-none border-[1px] border-solid w-full mb-[3vw] rounded-[10px] h-10"
    ...
    <div class="w-full flex justify-center">
      <button
        class="w-[30%] m-auto bg-[#BCB8B8] p-1 text-center rounded-[10px] mx-auto mt-3 text-white"
        type="button"
        @click="onSubmit"
      >
        作る
...
const schema = yup.object({
  name: yup
    .string()
    .max(255, validateMes.value.max)
    .required(validateMes.value.required)
    .matches(validateMes.value.regex, validateMes.value.regexMes)
    .trim(),
  password: yup
    .string()
    .max(20, validateMes.value.max)
    .matches(validateMes.value.regex, validateMes.value.regexMes),
})
useForm({
...
const onSubmit = handleSubmit(async (values) => {
  ...
  if (authState.value) {
    ;[makeBoardRes, statusCode.value] = await $makeBoard(
      values.name,
      values.password,
      useCookie<{ token: string }>("token").value.token
    )
  } else {
  ...
  switch (statusCode.value) {
    ...
    default:
      router.push({
        path: "board",
        query: { id: makeBoardRes.id, name: values.name },
      })
  }
  return
})

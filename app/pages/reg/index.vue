  ...
  <form class="w-[50%] m-auto text-[22px] pb-[9vh]">
    ...
    <div class="mt-12">
      ...
      <input
        type="text"
        name=""
        class="w-full border-[1px] border-[#D9D7D7] border-solid outline-none p-1 bg-white text-[#6B6666] inputFoucus"
        style="border-radius: 5px"
        v-model="name"
      />
    </div>
    <div class="mt-12">
      <p class="text-[#ff0000] text-[12px]" v-if="code === http.BadRequest">
        メールアドレスがすでに使われています
      </p>
...
const schema = yup.object({
  mail: yup
    ...
    .email(validateMes.value.mail)
    .matches(validateMes.value.regex, validateMes.value.regexMes)
    .trim(),
  name: yup
    .string()
    .max(10, validateMes.value.max)
    .min(2, validateMes.value.min)
    .required(validateMes.value.required)
    .matches(validateMes.value.regex, validateMes.value.regexMes),
  password: yup
    .string()
    .min(10, validateMes.value.min)
  ...
  conPassword: yup
    ...
    .matches(validateMes.value.regex, validateMes.value.regexMes)
    .required(validateMes.value.required)
    .oneOf([yup.ref("password")], validateMes.value.match),
})
useForm({

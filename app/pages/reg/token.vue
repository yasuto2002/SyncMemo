<template>
  <div class="pb-[5vw]">
    <div
      class="w-[40%] m-auto text-center text-[25px] text-[#BCB8B8] border-[1px] border-[#D9D7D7] border-solid p-[3vw] shadow-inner"
      style="border-radius: 5px"
    >
      <h1 class="mb-[2vw]">認証コードを送信しました。</h1>
      <p class="mb-[2vw]">
        メールアドレス <span>{{ mail }}</span> の認証を行います。
      </p>
      ...
      >
        ...
        <p class="text-[#ff0000] text-[12px]" v-if="result === false">
          コードが間違っています
        </p>
        <input
          type="number"
          name=""
          class="w-[90%] border-[1px] border-[#D9D7D7] border-solid outline-none p-1 bg-white text-[#6B6666] inputFoucus mx-auto mb-[2vw]"
          style="border-radius: 5px"
          autocomplete="on"
          max="9999"
          min="1111"
        ...
        />
        <button
          class="bg-white py-[3%] px-[5%] text-[20px] hover:shadow-none shadow-sm"
          @click="onSubmit"
        >
          認証を行う
        </button>
...
const schema = yup.object({
  token: yup
    .number()
    .max(9999, validateMes.value.max)

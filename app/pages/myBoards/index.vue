  ...
  <div class="m-auto pb-12">
    ...
    <TemplatePagination>
      <PartsPaginationBtn
        ...
        v-for="j in calc()"
        key="j"
        :class="{ 'bg-[#F9FAF7] hover:cursor-default': pageCount === j }"
        @click="pageCalc(j)"
        >{{ String(j) }}</PartsPaginationBtn
      >
    </TemplatePagination>
  </div>
</template>
...
const pageCalc = (num: number) => {
  if (num === pageCount.value) {
    ...
    arryCalc()
  } else if (num * defoCount.value - 1 < boards.value.length) {
    //ボードの数が６の倍数に満たない時
    viewBoard.value = boards.value.slice(
      (num - 1) * defoCount.value,
      boards.value.length
    )
  } else {
    viewBoard.value = boards.value.slice(
